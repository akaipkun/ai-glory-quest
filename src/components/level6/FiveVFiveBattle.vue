...
const heroes = [
  { emoji: '🛡️', name: '坦克', hp: 100 },
  { emoji: '🗡️', name: '刺客', hp: 70 },
  { emoji: '🔮', name: '法师', hp: 60 },
  { emoji: '🏹', name: '射手', hp: 65 },
  { emoji: '💚', name: '辅助', hp: 80 }
]
...
const enemyHeroes = [
  { emoji: '👹', name: 'Boss坦', hp: 110 },
  { emoji: '👺', name: 'Boss刺', hp: 75 },
  { emoji: '👿', name: 'Boss法', hp: 65 },
  { emoji: '💀', name: 'Boss射', hp: 70 },
  { emoji: '👻', name: 'Boss辅', hp: 85 }
]
...
function startBattle() {
  ...
  playerTeam.value = heroes.map(h => ({ ...h, currentHp: h.hp, maxHp: h.hp }))
  enemyTeam.value = enemyHeroes.map(h => ({ ...h, currentHp: h.hp, maxHp: h.hp }))
  ...
  battleTimer = setInterval(() => {
    ...
    // Simulate HP changes
    const playerDmg = calculatePlayerDamage()
    const enemyDmg = calculateEnemyDamage()

    // Damage a random alive enemy
    const aliveEnemies = enemyTeam.value.filter(e => e.currentHp > 0)
    if (aliveEnemies.length > 0) {
      const target = aliveEnemies[Math.floor(Math.random() * aliveEnemies.length)]
      target.currentHp = Math.max(0, target.currentHp - playerDmg)
    }

    const alivePlayers = playerTeam.value.filter(p => p.currentHp > 0)
    ...
    // Draw battle
    if (duration % 2 === 0) drawBattlefield()
  }, 300)
}
...
function calculatePlayerDamage() {
  ...
  const coopBonus = cooperation.value > 6 ? 3 : 0
  return base + aggBonus + coopBonus + Math.random() * 5
}
...
function calculateEnemyDamage() {
  ...
  return base + enemyStrength + Math.random() * 5
}
...
function drawBattlefield() {
  ...
  ctx.moveTo(w / 2, 20)
  ctx.lineTo(w / 2, 280)
  ctx.strokeStyle = 'rgba(194, 58, 43, 0.1)'
  ctx.lineWidth = 1
  ctx.setLineDash([6, 6])
  ctx.stroke()
  ctx.setLineDash([])
  ...
  ctx.fillText('你的队伍', w * 0.25, 12)
  ctx.fillText('系统AI', w * 0.75, 12)

  // Draw player team (left side)
  const team = playerTeam.value.length > 0 ? playerTeam.value : heroes
  team.forEach((hero, i) => {
    const x = w * 0.15 + Math.sin(Date.now() / 500 + i) * 10
    const y = 50 + i * 50
    ...
    if (hero.currentHp !== undefined) {
      ...
      ctx.fillRect(x - hpW / 2, y + 16, hpW, 4)
      const hpPct = hero.currentHp / hero.maxHp
      ctx.fillStyle = hpPct > 0.5 ? 'var(--verdant)' : hpPct > 0.2 ? 'var(--gold)' : 'var(--cinnabar)'
      ctx.fillRect(x - hpW / 2, y + 16, hpW * hpPct, 4)
    }
  ...
  eTeam.forEach((hero, i) => {
    const x = w * 0.85 - Math.sin(Date.now() / 500 + i) * 10
    const y = 50 + i * 50
    const alive = hero.currentHp === undefined || hero.currentHp > 0
  ...
  if (battleInProgress.value) {
    for (let i = 0; i < 5; i++) {
      ...
      ctx.fillStyle = `rgba(194, 58, 43, ${Math.random() * 0.3})`
      ctx.fill()
    }
  }
}
...
.b5__canvas { display: block; width: 100%; height: 300px; }
